...
class MainController:
    ...
    def check_data_ready(self):
        ...
        # First check data in data_controller
        if hasattr(self, 'data_controller') and hasattr(self.data_controller, 'data_model'):
            if self.data_controller.data_model.data is not None:
                return True
        
        # Check legacy data storage methods
        if self.current_data is None:
            if hasattr(self.view, 'spectra_data') and self.view.spectra_data is not None:
    ...
    def update_data_flow(self, data, step):
        if step == 'load':
            ...
            # Ensure data model is also updated
            if hasattr(self, 'data_controller') and hasattr(self.data_controller, 'data_model'):
                self.data_controller.data_model.data = data
