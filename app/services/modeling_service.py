...
class ModelingService:
    def __init__(self, plugins: dict):
        self.plugins = plugins  # Dynamically loaded plugins dictionary
        # Global random_state management (Requirement E)
    ...
    def create_leak_proof_pipeline(self, model_name: str, params: dict, task_type: str,
        ...
        """Create leak-proof Pipeline (Requirement E)
        
        Args:
            model_name: Model name
            params: Model parameters
            task_type: Task type ('classification' or 'regression')
            enable_scaling: Whether to enable standardization
        ...
        # Step 1: Standardization/Scaling (executed within CV)
        if enable_scaling:
            steps.append(('scaler', StandardScaler()))
        
        # Step 2: Feature selection (executed within CV)
        ...
        # Step 3: Model (ensure global random_state is used)
    ...
    def train_model_with_cv(self, model_name: str, params: dict, X: pd.DataFrame, y: pd.Series,
        ...
        """Train model using cross-validation, ensure leak-proof (Requirement E)
        ...
        Args:
            ...
            hyperopt_params: Hyperparameter optimization parameters
        ...
        # Handle labels (classification task)
        if task_type == 'classification':
        ...
        if hyperopt_params and hyperopt_params.get('enable', False):
            ...
            # Add prefix for Pipeline parameters
            pipeline_param_grid = {}
    ...
    def train_model(self, model_name: str, params: dict, X: pd.DataFrame, y: pd.Series):
        ...
        if model_name in self.plugins:
            algorithm = self.plugins[model_name]
        elif hasattr(self, 'view') and self.view and hasattr(self.view, 'plugins'):
            if model_name in self.view.plugins:
                algorithm = self.view.plugins[model_name]
            else:
                # Try fuzzy matching
                algorithm = find_algorithm_by_name(model_name, self.view.plugins, exact_match=False)
        
        # Also try fuzzy matching in service plugins if exact match failed
        if algorithm is None:
            algorithm = find_algorithm_by_name(model_name, self.plugins, exact_match=False)
        
        # V1.4.1: Debug and validate algorithm instance
        if algorithm is not None:
            print(f"🔍 Found algorithm for model '{model_name}': type={type(algorithm)}")
            # Check if algorithm is a class instead of instance
            if isinstance(algorithm, type):
                print(f"❌ ERROR: Algorithm '{model_name}' is stored as class, not instance!")
                print(f"   Class: {algorithm.__name__}")
                print(f"   Available methods: {[m for m in dir(algorithm) if not m.startswith('_')]}")
                raise ModelingError(f"Algorithm '{model_name}' is stored as class instead of instance. Please check plugin loading logic.")
            
            # Verify algorithm has required method
            if not hasattr(algorithm, 'train'):
                print(f"❌ ERROR: Algorithm '{model_name}' does not have 'train' method!")
                print(f"   Available methods: {[m for m in dir(algorithm) if not m.startswith('_')]}")
                raise ModelingError(f"Algorithm '{model_name}' does not have 'train' method.")
        
        if algorithm is not None:
            # Use plugin algorithm
            try:
                print(f"🔧 Training custom algorithm: {model_name}")
                print(f"   Input shape: X={X.shape}, y={len(y)}")
                print(f"   Parameters: {params}")
                
                model = algorithm.train(X, y, params)
                
                # 🔍 V1.4.1: Validate model object
                if model is None:
                    raise ModelingError(f"Algorithm '{model_name}' train() method returned None")
                
                # Check if model has predict method (for custom algorithms)
                if not hasattr(model, 'predict'):
                    # For plugin algorithms, use algorithm's predict method
                    if not hasattr(algorithm, 'predict'):
                        raise ModelingError(f"Algorithm '{model_name}' does not have predict method")
                    # Store algorithm reference in model for prediction
                    model._algorithm = algorithm
                
                # Set task type
                if hasattr(algorithm, 'get_task_type'):
                    task_type = algorithm.get_task_type()
                else:
                    task_type = "classification" if is_classification else "regression"
                
                setattr(model, 'task_type', task_type)
                print(f"✅ Algorithm '{model_name}' training completed")
                ...
                error_msg = f"Error training modeling algorithm '{model_name}': {str(e)}"
                print(f"❌ {error_msg}")
                import traceback
                traceback.print_exc()
                raise ModelingError(error_msg)
        else:
            ...
            try:
                if model_name == "Logistic Regression":
                    if not is_classification:
                        raise ModelingError("Logistic Regression only supports classification tasks.")
                ...
                elif hasattr(y, 'dtype') and y.dtype.kind == 'f':  # Float type
    ...
    def predict(self, model_id: str, X: np.ndarray) -> Dict[str, Any]:
        """
        Use specified model for prediction
        ...
        try:
            ...
            if not model_data or not hasattr(model_data, 'model') or model_data.model is None:
                return {"error": "Model does not exist or is not trained"}
            ...
            # **CRITICAL FIX: Check and use stored LabelEncoder for inverse transformation**
            if hasattr(model, '_label_encoder'):
                ...
                try:
                    ...
                    # Continue using original prediction results
            ...
            # If encoder not found, try generic processing
            try:
                ...
                try:
                    ...
                    return {
                        "predictions": predictions_list,
                        "note": "Prediction results converted to strings",
                        "error_detail": str(proc_error),
                        "label_conversion": "fallback"
                    }
                except:
                    return {"error": "Unable to process prediction results", "detail": str(proc_error)}
    ...
    def evaluate_model(self, model, X_test, y_test, task_type=None):
        """
        Compatibility method - provides same interface as evaluation_service.evaluate_model
        
        This is a bridge method to resolve the issue where controller calls evaluate_model but service only has evaluate method.
        This method processes parameters into the format required by evaluate method and calls it.
        
        Args:
            model: Trained model
            X_test: Test features
            y_test: Test labels
            task_type: Task type (optional, classification or regression)
