...
class ONNXService:
    ...
    def export_model_to_onnx(self, model: Any, X_sample: np.ndarray, 
        ...
        """
        try:
            logger.info("🔧 尝试ONNX格式导出...")
            
            # 暂时禁用ONNX转换，因为skl2onnx在当前环境中有问题
            logger.warning("⚠️ ONNX转换暂时禁用，因为skl2onnx库存在兼容性问题")
            logger.info("💡 将使用pickle格式保存模型")
            return None
    ...
    def save_model_as_pickle(self, model: Any, X_sample: np.ndarray, 
        ...
        try:
            ...
            # 保存模型
            logger.info(f"💾 保存pickle模型到: {filepath}")
