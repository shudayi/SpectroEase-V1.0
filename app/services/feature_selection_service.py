...
# Import new wavelength selection algorithms
from app.algorithms.wavelength_selection import CARS, SPA
...
class FeatureSelectionService:
    ...
    def select_features(self, method: str, params: dict, X, y) -> list:
        ...
        try:
            ...
            # V1.4.1: Use normalized name matching for better compatibility
            from app.utils.algorithm_name_matcher import find_algorithm_by_name
            
            algorithm = None
            # Try exact match first
            if method in self.plugins:
                algorithm = self.plugins[method]
            elif hasattr(self, 'view') and self.view and hasattr(self.view, 'plugins'):
                if method in self.view.plugins:
                    algorithm = self.view.plugins[method]
                else:
                    # Try fuzzy matching
                    algorithm = find_algorithm_by_name(method, self.view.plugins, exact_match=False)
            
            # Also try fuzzy matching in service plugins if exact match failed
            if algorithm is None:
                algorithm = find_algorithm_by_name(method, self.plugins, exact_match=False)
            
            # V1.4.1: Debug and validate algorithm instance
            if algorithm is not None:
                print(f"🔍 Found algorithm for method '{method}': type={type(algorithm)}")
                # Check if algorithm is a class instead of instance
                if isinstance(algorithm, type):
                    print(f"❌ ERROR: Algorithm '{method}' is stored as class, not instance!")
                    print(f"   Class: {algorithm.__name__}")
                    print(f"   Available methods: {[m for m in dir(algorithm) if not m.startswith('_')]}")
                    raise FeatureSelectionError(f"Algorithm '{method}' is stored as class instead of instance. Please check plugin loading logic.")
                
                # Verify algorithm has required method
                if not hasattr(algorithm, 'select_features'):
                    print(f"❌ ERROR: Algorithm '{method}' does not have 'select_features' method!")
                    print(f"   Available methods: {[m for m in dir(algorithm) if not m.startswith('_')]}")
                    raise FeatureSelectionError(f"Algorithm '{method}' does not have 'select_features' method.")
            
            if algorithm is not None:
                # Use plugin algorithm
                try:
                    selected_features = algorithm.select_features(X, y, params)
                    
                    # 🔍 V1.4.1: Validate return format
                    if not isinstance(selected_features, (list, tuple, np.ndarray, pd.Index)):
                        raise FeatureSelectionError(f"Algorithm '{method}' returned invalid type: {type(selected_features)}. Expected list of feature names.")
                    
                    # Convert to list
                    if isinstance(selected_features, (np.ndarray, pd.Index)):
                        selected_features = selected_features.tolist()
                    elif isinstance(selected_features, tuple):
                        selected_features = list(selected_features)
                    
                    # Validate feature names exist in DataFrame
                    missing_features = [f for f in selected_features if f not in X.columns]
                    if missing_features:
                        print(f"⚠️  WARNING: Algorithm '{method}' returned features not in DataFrame:")
                        print(f"   Missing features: {missing_features[:10]}")
                        print(f"   Available features: {list(X.columns[:5])}...")
                        # Filter out missing features
                        selected_features = [f for f in selected_features if f in X.columns]
                    
                    if len(selected_features) == 0:
                        raise FeatureSelectionError(f"Algorithm '{method}' returned no valid features.")
                    
                    print(f"✅ Algorithm '{method}' selected {len(selected_features)} features")
                    return selected_features
                    
                except Exception as e:
                    error_msg = f"Error executing feature selection algorithm '{method}': {str(e)}"
                    print(f"❌ {error_msg}")
                    import traceback
                    traceback.print_exc()
                ...
                if method == "SelectKBest" or method == "DELETED_SelectKBest":
                    k = params.get('k', 10)
                    ...
                    try:
                        ...
                        if numeric_encoding_needed:
                            ...
                            raise
                elif method == "RFE" or method == "Recursive Feature Elimination (RFE)" or method == "DELETED_Recursive Feature Elimination (RFE)":
                    try:
                        n_features_to_select = params.get('n_features_to_select', params.get('k', 10))
                        
                        # Ensure does not exceed available features
                        ...
                        if is_classification:
                            ...
                            # Reduce estimator count to improve speed and stability
                        ...
                        # Use larger step to speed up
                        step = max(1, int(X.shape[1] * 0.05))  # Eliminate 5% of features each time
                        ...
                        if is_classification:
                            ...
                            # Ensure X is numpy array format
                            X_array = X.values if hasattr(X, 'values') else X
                            selector = selector.fit(X_array, y_encoded)
                        else:
                            X_array = X.values if hasattr(X, 'values') else X
                        ...
                        print(f"❌ RFE feature selection failed: {e}")
                        import traceback as tb
                        tb.print_exc()
                        raise FeatureSelectionError(f"RFE feature selection failed: {e}")
                elif method == "Feature Importance" or method == "DELETED_Feature Importance":
                ...
                elif method == "Mutual Information" or method == "DELETED_Mutual Information":
                ...
                elif method == "LASSO" or method == "DELETED_LASSO":
                    C = params.get('alpha', 1.0)
                    k = params.get('k', 10)  # Number of features user expects to select
                    ...
                    # Select top k features by coefficient magnitude (including zero coefficients)
                    k = min(k, X.shape[1])
                    top_indices = np.argsort(coefficients)[-k:]
                    selected_features = X.columns[top_indices]
                    
                    print(f"   LASSO selected {len(selected_features)} features")
                    non_zero_count = np.sum(coefficients[top_indices] > 1e-10)
                ...
                # It has been moved to Feature Extraction Service for proper unsupervised use
                    ...
                    # CARS (Competitive Adaptive Reweighted Sampling) - Standard for spectral analysis
                    try:
                        # Get parameters
                        n_iterations = params.get('n_iterations', 50)
                        pls_components = params.get('pls_components', 5)
                        n_folds = params.get('n_folds', 5)
                        sampling_ratio = params.get('sampling_ratio', 0.9)
                        k = params.get('k', None)  # Number of features user expects
                        ...
                        # Check task type and prepare y
                        if self._is_classification_task(y):
                            # Classification task: CARS is designed for regression
                            print("   Warning: CARS is designed for regression tasks")
                            from sklearn.preprocessing import LabelEncoder
                            le = LabelEncoder()
                            if isinstance(y, pd.Series):
                                y_numeric = le.fit_transform(y.astype(str))
                            else:
                                y_numeric = le.fit_transform([str(label) for label in y])
                        else:
                            # Regression task: use numeric values directly
                            y_numeric = pd.to_numeric(y, errors='coerce')
                            if y_numeric.isna().any():
                                raise FeatureSelectionError("CARS requires numeric target variable")
                        
                        # Fit CARS
                        cars.fit(X.values, y_numeric)
                        
                        # Get selected wavelength indices
                        selected_indices = cars.selected_indices_
                        
                        # If user specified k parameter, select top k from CARS selected features
                        if k is not None and k < len(selected_indices):
                            # Use absolute value of PLS coefficients to rank feature importance
                            from sklearn.cross_decomposition import PLSRegression
                            X_selected = X.values[:, selected_indices]
                            n_comp = min(pls_components, X_selected.shape[1], X_selected.shape[0] - 1)
                            pls = PLSRegression(n_components=n_comp)
                            pls.fit(X_selected, y_numeric)
                            coef_abs = np.abs(pls.coef_.ravel())
                            top_k_local_indices = np.argsort(coef_abs)[-k:]
                            selected_indices = selected_indices[top_k_local_indices]
                        
                        selected_features = X.columns[selected_indices]
                        return list(selected_features)
                        
                    except Exception as e:
                        print(f"❌ CARS wavelength selection failed: {e}")
                        import traceback as tb
                        tb.print_exc()
                        raise FeatureSelectionError(f"CARS wavelength selection failed: {e}")
                
                elif "SPA" in method:
                    # SPA (Successive Projections Algorithm) - Fast wavelength selection
                    try:
                        # Get parameters - prefer k parameter, then n_wavelengths
                        k = params.get('k', params.get('n_wavelengths', 20))
                        n_wavelengths = min(k, X.shape[1])  # Ensure does not exceed available wavelengths
                        min_wavelengths = params.get('min_wavelengths', 5)
                        max_wavelengths = params.get('max_wavelengths', min(X.shape[1], 50))
                        ...
                        # Fit SPA (SPA is unsupervised, does not need y)
                        spa.fit(X.values)
                        
                        # Get selected wavelength indices
                        selected_indices = spa.selected_indices_
                        selected_features = X.columns[selected_indices]
                        ...
                        print(f"❌ SPA wavelength selection failed: {e}")
                        import traceback as tb
                        tb.print_exc()
                        raise FeatureSelectionError(f"SPA wavelength selection failed: {e}")
                    ...
                    k = params.get('k', min(30, X.shape[1]))
    ...
    def apply_method(self, X_train, y_train, X_test=None, method=None, params=None):
        ...
        try:
            ...
            print(f"   Expected features to select: {n_features}")
            ...
            if not selected_features or len(selected_features) == 0:
                print("⚠️  Warning: Feature selection returned empty result, will use all features")
            ...
            print(f"   Input features: {X_train.shape[1]}")
            print(f"   Expected to select: {n_features} features")
            print(f"   Actually selected: {len(selected_features)} features")
            print(f"   Selection rate: {len(selected_features)/X_train.shape[1]*100:.1f}%")
            if len(selected_features) != n_features:
                print(f"   ⚠️  Warning: Actual selection count does not match expected!")
            print(f"{'='*80}")
            print(f"")
            ...
            # 🔧 V1.2.1 Fix: Use original data to calculate importance, avoid using filtered data
            print("🎯 Calculating feature importance using full feature set...")
            
            # Get full feature set data (data before feature selection)
            if original_X_train_type == np.ndarray:
                X_for_importance = original_X_train if 'original_X_train' in locals() else X_train.values
            else:
                X_for_importance = X_train
            
            print(f"   Using X_train for importance: shape={X_for_importance.shape}")
            
            try:
                if isinstance(X_for_importance, pd.DataFrame):
                    data_min = float(X_for_importance.min().min())
                    data_max = float(X_for_importance.max().max())
                else:
                    data_min = float(X_for_importance.min())
                    data_max = float(X_for_importance.max())
                print(f"   Data range: [{data_min:.4f}, {data_max:.4f}]")
            except Exception as fmt_error:
                print(f"   Data range formatting error: {fmt_error}")
            
            feature_importances = self._calculate_feature_importance(X_for_importance, y_train, method, task_type=task_type)
    ...
    def _calculate_feature_importance(self, X, y, method, task_type='classification', **kwargs):
        ...
        def smart_preprocessing(X):
            ...
            # 🔧 V1.2.1 Fix: Improve detection logic to avoid duplicate preprocessing
            is_already_standardized = (
                (-5 < data_min_val < 0) and 
                (0 < data_max_val < 5) and 
                (abs(data_mean_val) < 1.0) and 
                (0.5 < data_std_val < 2.0)
            )
            
            is_already_normalized = (
                (-0.1 <= data_min_val <= 0.1) and 
                (0.9 <= data_max_val <= 1.1)
            )
            
            if is_already_standardized:
                print("✅ Data appears already standardized - using original data")
                return X, None, None
            elif is_already_normalized:
                print("✅ Data appears already normalized - using original data")
                return X, None, None
            elif data_range < 0.01:
                print("⚠️  Data has very small range - may have been over-processed")
                print("   Using original data to preserve information")
                return X, None, None
            elif data_range > 1000:
                print("🔧 Large data range detected - applying gentle robust scaling")
        ...
        if method in ['Feature Importance', 'Random Forest']:
            try:
                ...
                # 🔧 V1.2.1 Optimization: Random Forest parameters optimized for spectral data
                if task_type == 'classification':
                    from sklearn.ensemble import RandomForestClassifier
                    model = RandomForestClassifier(
                        n_estimators=200,
                        max_depth=15,            # Limit depth to prevent overfitting
                        min_samples_split=5,     # Increase stability
                        min_samples_leaf=2,      # Prevent overfitting
                        max_features=0.3,        # More suitable for spectral data (30% features)
                        bootstrap=True,
                        random_state=42,
                        class_weight='balanced',
                        n_jobs=-1
                    )
                    print(f"   🌲 RF parameters: depth=15, min_split=5, min_leaf=2, max_features=30%")
                else:
                    from sklearn.ensemble import RandomForestRegressor
                    model = RandomForestRegressor(
                        ...
                        min_samples_split=5,     # Increase stability
                        min_samples_leaf=2,      # Prevent overfitting
                        max_features=0.3,        # More suitable for spectral data
                        bootstrap=True,
                        random_state=42,
                        n_jobs=-1
                    )
