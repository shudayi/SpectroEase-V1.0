...
class PreprocessingService:
    ...
    def despiking(self, data, method='mad', window=11, threshold=5, **kwargs):
        """Apply despiking (spike removal) to spectral data - 拉曼去峰处理
        
        Args:
            data: Input spectral data (DataFrame)
            method: Despiking method ('mad' or 'localz')
            window: Window size for spike detection (7-15)
            threshold: Threshold for spike detection (3-6)
        ...
        try:
            ...
            # Validate parameters
            window = max(7, min(15, int(window)))
            if window % 2 == 0:
                window += 1  # Ensure odd window size
            threshold = max(3, min(6, float(threshold)))
