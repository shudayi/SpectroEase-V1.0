...
class EvaluationService:
    ...
    def eval_classification_metrics(self, y_true, y_pred, metrics: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'confusion_matrix' in metrics:
                ...
                try:
                    # **修复混淆矩阵显示问题：根据Number of classes量动态调整图形大小**
                    n_classes = len(np.unique(np.concatenate([y_true_idx, y_pred_idx])))
                    
                    # 根据Number of classes量动态调整图形大小
                    if n_classes <= 5:
                        figsize = (8, 6)
                        font_size = 12
                    elif n_classes <= 10:
    ...
    def generate_reproducibility_info(self, model=None, preprocessing_params=None, 
        ...
        Args:
            model: 训练好的模型
            preprocessing_params: 预处理Parameters
            feature_selection_params: 特征选择Parameters  
            cv_results: 交叉验证结果
