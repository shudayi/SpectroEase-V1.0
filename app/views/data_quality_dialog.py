...
class DataQualityDialog(QDialog):
    ...
    def preview_changes(self):
        ...
        try:
            ...
            if remaining_analysis['severity'] == 'none':
                ...
                preview_text += f"⚠️ Remaining issues (severity: {remaining_analysis['severity']})\n"
    ...
    def _apply_removal(self, data: pd.DataFrame, preview: bool = False) -> pd.DataFrame:
        ...
        if self.remove_cols_check.isChecked():
            ...
            if not preview:
                print(f"🗑️ Removed {len(cols_to_remove)} columns with >{threshold:.0%} NaN values")
        ...
        if self.remove_rows_check.isChecked():
            ...
            if not preview:
                print(f"🗑️ Removed {removed_rows} rows with NaN values")
