    ...
    def init_param_widgets(self, method):
        ...
        if method in self.plugins:
            ...
            if method == "Train-Test Split":
                # Use QGridLayout for a more organized and responsive layout
                params_grid = QGridLayout()
                params_grid.setSpacing(8)

                # Row 0: Test Size and Random Seed
                test_size_label = QLabel("Test Size:")
                self.test_size_spin = QDoubleSpinBox()
                self.test_size_spin.setRange(0.01, 0.99)
                self.test_size_spin.setValue(0.2)
                self.test_size_spin.setSingleStep(0.05)
                params_grid.addWidget(test_size_label, 0, 0)
                params_grid.addWidget(self.test_size_spin, 0, 1)

                random_label = QLabel("Random Seed:")
                self.random_state_spin = QSpinBox()
                self.random_state_spin.setRange(0, 10000)
                self.random_state_spin.setValue(42)
                params_grid.addWidget(random_label, 0, 2)
                params_grid.addWidget(self.random_state_spin, 0, 3)

                # Row 1: Min Samples and Shuffle
                min_samples_label = QLabel("Min Samples:")
                self.min_samples_spin = QSpinBox()
                self.min_samples_spin.setRange(1, 100)
                self.min_samples_spin.setValue(5)
                params_grid.addWidget(min_samples_label, 1, 0)
                ...
                params_grid.addWidget(shuffle_label, 1, 2)
                params_grid.addWidget(self.shuffle_checkbox, 1, 3, Qt.AlignLeft)

                # Row 2: Stratify
                stratify_label = QLabel("Stratify:")
                self.stratify_combo = QComboBox()
                self.stratify_combo.addItem("None")
                self.stratify_combo.addItem("label")  # If 'label' column exists
                params_grid.addWidget(stratify_label, 2, 0)
                params_grid.addWidget(self.stratify_combo, 2, 1, 1, 3) # Span across 3 columns

                # Set column stretch to ensure widgets don't spread out too much, while allowing expansion
                params_grid.setColumnStretch(1, 1)
                params_grid.setColumnStretch(3, 1)
                params_grid.setColumnStretch(4, 10) # Add a spacer column to push widgets to the left

                self.params_form.addRow(params_grid)
