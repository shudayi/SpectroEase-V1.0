        ...
        def safe_numeric_check(labels):
            ...
            try:
                ...
                if string_label_count > len(sample_labels) * 0.5:
                    # 超过一半是明显的字符串标签，跳过数值转换
                    logger.info("检测到字符串标签，跳过数值转换检查")
                    return None, 1.0  # 返回None和高NaN比率表示非数值
