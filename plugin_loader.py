...
def load_plugins(plugin_base_dir: str) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    ...
    if not os.path.exists(plugin_base_dir):
        # print(f"Warning: Plugin directory {plugin_base_dir} not found. Using empty plugin collections.")  # 禁用输出
    ...
    for subdir_name, plugin_dict in plugin_dirs.items():
        ...
        if not os.path.exists(subdir_path):
            # print(f"Warning: Plugin subdirectory {subdir_path} not found. Skipping.")  # 禁用输出
            continue
                    ...
                    try:
                        ...
                        for name, obj in inspect.getmembers(module, inspect.isclass):
                            ...
                            if _is_plugin_class(obj, subdir_name):
                                ...
                                # print(f"Loaded {subdir_name} plugin: {name}")  # 禁用输出
                    
                    except Exception as e:
                        # print(f"Warning: Failed to load plugin {filename} from {subdir_name}: {e}")  # 禁用输出
                        continue
        
        except Exception as e:
            # print(f"Warning: Failed to scan plugin directory {subdir_path}: {e}")  # 禁用输出
            continue
    ...
    # print(f"Plugin loading summary:")  # 禁用输出
    # print(f"  - Preprocessing: {len(preprocessing_plugins)} plugins")  # 禁用输出
    # print(f"  - Feature Selection: {len(feature_selection_plugins)} plugins")  # 禁用输出
    # print(f"  - Modeling: {len(modeling_plugins)} plugins")  # 禁用输出
    # print(f"  - Data Partitioning: {len(data_partitioning_plugins)} plugins")  # 禁用输出
...
if __name__ == "__main__":
    ...
    # print("\nPlugin loading test completed.")  # 禁用输出
